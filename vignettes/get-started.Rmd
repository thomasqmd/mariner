...
packaging Quarto (.qmd) documents.

The core workflow involves two main steps:

1.  **`generate_reports()`**: Create multiple, parameterized `.qmd` source files from a single template.
...
You can also use your own `.qmd` file as a template by providing a direct path to it using the `template_path` argument.
