...
#'
#' Copies the theme's font files into the current user's font directory, so that
#' Atkinson Hyperlegible, Lora and JetBrains Mono are available to every program
#' on the machine -- including the graphics devices that draw figures for PDF
#' output.
#'
...
#'
#' Check with `mariner_check_setup()` (or `mariner_fonts_available("pdf")`)
#' whether this applies to your machine.
#'
#' @section Platform notes:
#' Nothing here needs administrator rights -- the fonts go into your own user
#' font directory, not the system one.
#'
#' * **macOS** copies to `~/Library/Fonts`; available immediately.
#' * **Linux** copies to `~/.local/share/fonts` and runs `fc-cache` if present.
#' * **Windows** copies to `%LOCALAPPDATA%\\Microsoft\\Windows\\Fonts` and
#'   registers each face under `HKEY_CURRENT_USER`. Copying alone is not enough
#'   on Windows -- an unregistered file in that directory is ignored -- so if the
#'   registry step fails, the function says so rather than reporting success.
#'
#' **Restart R afterwards.** Graphics devices read the font configuration once
#' per session, so a face installed mid-session is not picked up until R is
#' restarted.
#'
#' @param overwrite Replace font files that are already installed. Defaults to
#'   `FALSE`, so re-running is cheap and safe.
#' @param quiet Suppress the summary.
#' @return Invisibly, a character vector of the files installed (those actually
